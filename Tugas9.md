# Tugas 9 - Integrasi Final Full-Stack, Relasi Database, dan Geolocation

**Nama:** Khonsadiya Nasywa Salsabila
**NIM:** 20230140112 
**Kelas:** C 


---

## Screenshots Hasil Praktikum

### 1. Token ketika Login
![TokenLogin](screenshoots/LoginToken.png)

### 2. Bearer Token saat Check-In
![Bearer Token](screenshoots/Check-InBearerToken.png)

### 3. Latitude, Longitude ketika Check-In
![Latitude, Longitude](screenshoots/Check-inLatitude-longitude.png)

### 4. Login Mahasiswa
![Login Mahasiswa](screenshoots/LoginMahasiswa.png)

### 5. Check-In dengan map OSM
![map OSM Check-In](screenshoots/Check-InMapsOSM.png)

### 6. Check-Out dengan map OSM
![map OSM Check-Out](screenshoots/Check-OutMapsOSM.png)

### 7. Login Admin
![login Admin](screenshoots/LoginAdmin.png)

### 8. Report Berisi Data Presensi dari Semua User
![Report](screenshoots/ReportSemuaPresensi.png)

### 9. Database Tabel Presensi
![Database Presensi](screenshoots/DatabasePresensi.png)